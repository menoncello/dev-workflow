...
import { buildForEnvironment } from "../src/utils/build";
...
class QualityChecker {
  ...
  private async checkTypeScript(): Promise<void> {
    ...
    try {
      const _output = execSync("bun tsc --noEmit", {
        encoding: "utf8",
      ...
      if (this.metrics.typescript.compilationErrors > 0) {
        console.log(`❌ TypeScript errors found: ${this.metrics.typescript.compilationErrors}`);
      }
    }
  }
  ...
  private async checkBuilds(): Promise<void> {
    ...
    try {
      ...
      this.metrics.build.buildErrors.push(
  ...
  private async checkTests(): Promise<void> {
    ...
    try {
      ...
      try {
        const _coverageOutput = execSync("bun test --coverage", {
          encoding: "utf8",
          cwd: this.projectRoot,
        });
      ...
      if (passMatch) {
        this.metrics.tests.passingTests = Number.parseInt(passMatch[1]);
      }
      if (failMatch) {
        this.metrics.tests.failingTests = Number.parseInt(failMatch[1]);
      }
      this.metrics.tests.totalTests =
        this.metrics.tests.passingTests + this.metrics.tests.failingTests;

      console.log(`⚠️  Tests had issues: ${this.metrics.tests.failingTests} failing tests`);
    }
  }
  ...
  private async checkSecurity(): Promise<void> {
    ...
    try {
      ...
      if (vulnMatches) {
        this.metrics.security.vulnerabilities = Number.parseInt(vulnMatches[1]);
      }
  ...
  printReport(report: QualityReport): void {
    ...
    console.log(
      `  Linting: ${report.metrics.linting.errors} errors, ${report.metrics.linting.warnings} warnings`
    );
    console.log(
      `  Build: ${report.metrics.build.buildErrors.length} errors, ${report.metrics.build.buildSizeReduction}% size reduction`
    );
    console.log(
      `  Tests: ${report.metrics.tests.passingTests}/${report.metrics.tests.totalTests} passing`
    );
...
async function main() {
  ...
  const _showDetails = args.includes("--verbose") || args.includes("-v");
