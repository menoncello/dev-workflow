...
async function validateSingleEnvironment(environment: string): Promise<ValidationResult> {
  ...
  try {
    ...
    logger.warn(`Failed to read build directory: ${config.outdir}`, error as Error);
  }
...
function printReport(report: ValidationReport): void {
  ...
  console.log(`Total Build Time: ${(report.totalBuildTime / 1000).toFixed(2)}s`);
  console.log(`Successful Builds: ${report.summary.successful}/${report.environments.length}`);
  console.log(`Total Build Size: ${(report.summary.totalSize / 1024 / 1024).toFixed(2)} MB`);
  ...
  report.environments.forEach((r) => {
    ...
    const size = r.success ? `${(r.size / 1024 / 1024).toFixed(2)} MB` : "N/A";
  ...
  if (report.overall) {
    ...
    const prodResult = report.environments.find((r) => r.environment === "production");
    ...
    console.log("  ❌ Some builds are failing. Please check the errors above.");
  }

  // Check for consistency
  const sizes = report.environments.filter((r) => r.success).map((r) => r.size);
  ...
  if (sizes.length > 1) {
    ...
    if (sizeDifferencePercent > 50) {
      console.log("  📏 Large size variation between environments detected");
    }
  }
}
