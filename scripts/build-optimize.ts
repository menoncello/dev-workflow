...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (options.analyze) {
      ...
      const config = (await import("../src/config/build")).buildConfigs[options.environment];
      ...
      if (analysis.totalSize > 10 * 1024 * 1024) {
        // > 10MB
        console.log("  ⚠️  Build size is quite large. Consider code splitting.");
      }
      if (analysis.fileTypes[".js"] > 10) {
        console.log("  📦 Consider chunking large JavaScript files.");
      }
      if (!analysis.fileTypes[".map"]) {
        console.log("  🔍 Source maps are disabled. This is good for production.");
      }
    }
