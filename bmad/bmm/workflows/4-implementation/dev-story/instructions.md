...
Story is already marked in-progress
  ...
  <step n="4" goal="Run quality gates and validations">
    ...
    <substep n="4.2" goal="Biome validation">
      <action>Run: bun run lint</action>
      <check>ZERO Biome errors required</check>
    ...
    <substep n="4.3" goal="Code formatting check">
      ...
      <check>ZERO formatting errors required</check>
    </substep>

    <substep n="4.4" goal="Unit/integration tests">
      <action>Run: bun test</action>
      <check>100% test pass rate required - no failing tests allowed</check>
    </substep>

    <action>Validate implementation meets ALL story acceptance criteria; if ACs include quantitative thresholds, ensure they are met before marking complete</action>

    <check>If any quality gate fails → STOP and fix before continuing</check>
    <check>Only mark tasks complete when ALL quality gates pass</check>
  </step>
