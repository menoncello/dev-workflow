...
Will update existing story file rather than creating new one.
  ...
  <step n="8.5" goal="Validate Code Examples in Story">
    <critical>All code examples in stories must meet production quality standards</critical>

    <action>Scan story for code examples and validate:</action>
    <check>TypeScript types in examples compile with 0 errors</check>
    <check>Examples pass Biome linting with 0 errors</check>
    <check>Examples follow Biome formatting conventions</check>
    <action>Reject story with examples that violate quality standards</action>
    <action>Provide specific feedback on required fixes before story approval</action>
  </step>
