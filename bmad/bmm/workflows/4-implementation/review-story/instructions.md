...
{{result_story_list}}
  ...
  <step n="6.5" goal="Verify Quality Gates Compliance">
    <critical>Review must verify that quality gates were properly enforced</critical>

    <action>Scan all changed files for quality violations:</action>
    <check>Scan for biome-disable, eslint-disable, @ts-ignore, @ts-expect-error comments</check>
    <check>Verify TypeScript compilation was checked with 0 errors</check>
    <check>Verify Biome linting was run with 0 errors</check>
    <check>Verify all tests pass with 100% success rate</check>
    <check>Verify security audit was run with 0 vulnerabilities</check>
    <action>Flag any quality violations as High severity findings</action>
  </step>
