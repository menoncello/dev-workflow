...
export class MCPToolAdapter {
	...
	async executeTool(
		...
		try {
			...
			switch (tool.adapter.type) {
				...
				default:
					throw new MCPError(
						`Unsupported adapter type: ${tool.adapter.type}`,
						toolId,
					);
			}
	...
	private validateTool(tool: MCPTool): void {
		...
		if (
			!tool.configuration ||
			typeof tool.configuration.timeout !== "number"
		) {
			throw new MCPError("Invalid tool configuration", tool.id);
		}
	}
	...
	private async testAdapterConnection(
		adapter: import("../types/mcp").MCPAdapter,
	): Promise<void> {
