...
import type { PayloadCreateUser, QueryParamUser, QueryParamUsers } from "./models";

const getUsers = async (prisma: PrismaClient, query: QueryParamUsers) => {
  const { username, email, isActive, isDeleted, createdAt, updatedAt, limit, skip, desc, asc, q } =
    query;
  ...
  if (q) {
    const fieldSearchs: Prisma.UserOrderByRelevanceFieldEnum[] = ["email", "username"];
